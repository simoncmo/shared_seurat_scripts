...
make_NMK_STdim_plot = function(st_merge, st_male, st_female, group.by, male_sm_set, male_sm_set_ptsize, female_sm_set, female_sm_set_ptsize,color_palette='Seurat'){
	########### NMK ########################
	########### individual strips ########################
	# Spatial Dis plot

	
	p_nmk_M = SpatialPlot(st_male,
		group.by = group.by,
		images   = male_sm_set,
...
plot_NMK_AKI  = function(st_merge, st_male, st_female, st_aki, gene_to_plot, title ="", subtitle="", plot_group){
	...
	p_nmk_wrapped = if(gene_to_plot %in% rownames(st_merge)){
		make_NMK_STdim_plot(st_merge, st_male, st_female, gene_to_plot, male_sm_set, male_sm_set_ptsize, female_sm_set, female_sm_set_ptsize)
	}else{
...
############### DIMPLOTs from here ####################
#######################################################
SpatialDimPlotPlus = function(obj, group.by, images, pt.size.factors, labels, show_image = T, colors = plot_col){
    ### Prefly checks
    # Images
    if(missing(images)){ 
    ...
    # Labels
    if(missing(labels)){
        labels = rep('',length(images))
    }else if(length(labels)!=length(images)){
        message('Warning: labels lenght not equal to images length. Label with images names')
        labels = images
    }
    # pt size
    ...
    ## Set colors
    group_items = unique(unlist(obj[[group.by]]))
    if(missing(colors)){
        colors      = colors[1:length(group_items)] %>% setNames(group_items)
    }
    if(is.null(names(colors))){
        messge("Color vector missing name. Assign with elements in the obj")
        colors      = colors %>% setNames(group_items)
    }
    ### PLOT
    map(1:length(images), function(idx){
        p = SpatialPlot(obj, 
                    image.alpha = if(show_image) 1 else 0,
                    group.by = group.by, 
                    images = images[[idx]], 
                    pt.size.factor = pt.size.factors[[idx]],
                    stroke=NA) + 
        labs(title = labels[[idx]]) +
...
nmk_STDim_UMAP_plot = function(st_merge, st_male, st_female, group_by, male_sm_set, male_sm_set_ptsize, female_sm_set, female_sm_set_ptsize, age_labels, plot_colors){
    p_umap = DimPlot(st_merge, group.by = group_by) + labs(x='',y='',title = group_by) +
        theme(axis.text = element_blank(),
              axis.ticks= element_blank(), 
              axis.line = element_blank(),
             legend.position = 'none') + 
        scale_color_manual(values = plot_colors, limits = names(plot_colors))+
        coord_fixed()
    p_male = SpatialDimPlotPlus(st_male, 
                                group.by = group_by, 
                                images = male_sm_set, 
                                pt.size.factors = male_sm_set_ptsize,
                                labels = age_labels,
    ...
    p_female = SpatialDimPlotPlus(st_female, 
                                group.by = group_by, 
                                images = female_sm_set, 
                                pt.size.factors = female_sm_set_ptsize,
                                show_image=F,
    ...
    p_stnmk = wrap_plots(
        wrap_plots(p_male, nrow =1),
        wrap_plots(p_female, nrow =1),
        nrow =2,
        guides = 'collect'
