...
getRCTD_result = function(st_obj, snrna_obj, colu_cell_type='cell_type_Abbr', mode ='full',cell_count_filter = 25, reduce_size = F){
	...
	st_coords = map(Images(st_obj), ~ GetTissueCoordinates(st_obj, image =.x)) %>% 
			bind_rows() %>%
    ...
    ### REDUCE ONLY IF NEEDED
    RCTD_result = if(reduce_size) RCTD_obj2list(RCTD_obj) else RCTD_obj
